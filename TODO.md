...
- [ ] `[S]` Dense icon spacing slice 1 — leading disclosure controls. Target `bf-accordion-tab`, `bf-side-navigation-accordion-button`, and `bf-side-navigation-expand`; standardize the in-flow `1rem` chevron + text-gap contract and validate on the accordion/side-navigation demos.
