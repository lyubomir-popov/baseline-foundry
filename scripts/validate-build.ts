...
function validateCommonCss(css: string): void {
  ...
  assert(css.includes("--bf-h6-selected-start-nudge: var(--bf-h6-cap-start-nudge);"), "Expected compat CSS to expose h6 alignment through the cap-engine override.");
  ...
  assert(css.includes(":where(.p-form__group--checkbox, .bf-field.is-checkbox) :where(.p-form__control, .bf-control) {\n  gap: 0;"), "Expected checkbox field controls to avoid downstream gap overrides.");
  ...
  assert(css.includes(":where(.p-switch__slider, .bf-switch-slider)"), "Expected compat CSS to include switch styling.");
  ...
  assert(css.includes(":where(.p-card, .p-card--highlighted, .p-card--overlay, .p-card--muted, .bf-card, .bf-card.is-highlighted, .bf-card.is-overlay, .bf-card.is-muted)"), "Expected compat CSS to include card surfaces.");
  assert(css.includes(":where(.p-divider, .bf-divider)"), "Expected compat CSS to include divider surfaces.");
  assert(css.includes(":where(.p-segmented-control__button, .p-tab-buttons__button, .bf-segmented-control-button, .bf-tab-buttons-button)"), "Expected compat CSS to include segmented control buttons.");
  assert(css.includes(":where(.p-breadcrumbs__items, .bf-breadcrumbs-items)"), "Expected compat CSS to include breadcrumb styling.");
  assert(css.includes(":where(.p-pagination__items, .bf-pagination-items)"), "Expected compat CSS to include pagination styling.");
  assert(css.includes(":where(table, .p-table, .bf-table)"), "Expected compat CSS to include table styling.");
  assert(css.includes(":where(.p-chip, .p-chip--positive, .p-chip--caution, .p-chip--negative, .p-chip--information, .bf-chip, .bf-chip.is-positive, .bf-chip.is-caution, .bf-chip.is-negative, .bf-chip.is-information)"), "Expected compat CSS to include chip styling.");
  assert(css.includes(":where(.p-badge, .p-badge--negative, .bf-badge, .bf-badge.is-negative)"), "Expected compat CSS to include badge styling.");
  ...
  assert(css.includes(":where(.vr-code-snippet__block--icon, .p-code-snippet__block--icon, .bf-code-snippet-block.is-icon) {\n  cursor: copy;"), "Expected compat CSS to include copyable code-snippet blocks.");
  assert(css.includes(":where(.vr-list-tree, .p-list-tree, .bf-list-tree)"), "Expected compat CSS to include list-tree styling.");
  assert(css.includes(":where(.p-tabs--equal, .bf-tabs.is-equal)"), "Expected compat CSS to include equal-width dense tab modifiers.");
  ...
  assert(css.includes(":where(.p-panel__toggle, .bf-panel-toggle)"), "Expected compat CSS to include panel toggle styling.");
  assert(css.includes(":where(.l-application__overlay, .bf-application-overlay)"), "Expected compat CSS to include application drawer overlay styling.");
  ...
  assert(css.includes(":where(.l-application__aside-resize-handle, .bf-application-aside-resize-handle)"), "Expected compat CSS to include the pinned-aside resize handle selector.");
