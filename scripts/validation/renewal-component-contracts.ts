...
export function validateRenewalComponentContracts(
  ...
  assert(css.includes("--bf-icon-label-optical-offset-block: 0.1875rem;") && css.includes("--bf-side-navigation-icon-optical-offset-block: var(--bf-icon-label-optical-offset-block);") && css.includes("--bf-disclosure-icon-optical-offset-block: 0rem;"), "Expected side-navigation leading icons to retain their 0.1875rem optical offset while disclosure chevrons remain centred on the text line.");
  assert(css.includes("--bf-side-navigation-icon-gap: var(--bf-leading-mark-gap);"), "Expected icon-navigation to consume the shared Canonical mark/icon gap.");
  ...
  assert(css.includes("--bf-side-navigation-content-inset: var(--bf-component-inline-inset-continuation);") && css.includes("--bf-side-navigation-disclosure-inset: var(--bf-disclosure-group-inset);"), "Expected plain and disclosure side-navigation copy to resolve to the same continuation inset without introducing another component inset.");
  ...
  assert(css.includes(":not(:has(> .bf-side-navigation-icon))::before") && css.includes("flex: 0 0 var(--bf-disclosure-icon-inline-size);"), "Expected iconless rows in icon navigation to reserve the same named mark slot as icon-bearing rows.");
  ...
  assert(css.includes("container-name: bf-basic-section;") && css.includes("@container bf-basic-section (width >= 38.75rem)") && css.includes("grid-template-columns: repeat(2, minmax(0, 1fr));"), "Expected basic section to establish the standard medium 50/50 container-query breakpoint.");
  assert(css.includes("@container bf-basic-section (width >= 38.75rem)") && css.includes(".bf-basic-section-layout) {\n    column-gap: var(--bf-grid-gap-inline);"), "Expected basic section 50/50 layout rules to target the layout descendant at the medium breakpoint.");
  assert(!css.includes("@container bf-application") && !css.includes("@media (min-width: 48rem) {\n  :where(.bf-theme.bf-application) > :where(.bf-main) :where(.bf-basic-section-layout)"), "Expected application chrome not to override the intrinsic basic-section and tiered-list container queries.");
  ...
  assert(css.includes("@container (width >= 38.75rem)") && css.includes(".bf-tiered-list:not(.is-description-full-width)"), "Expected tiered-list headers to use the standard medium container breakpoint.");
  ...
  assert(css.includes(".bf-hero-chip.bf-chip") && css.includes("column-gap: var(--bf-leading-mark-gap);"), "Expected hero chip composition to use the shared mark/icon gap.");
