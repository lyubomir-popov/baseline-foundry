...
export function validateSpacingSpecPage(spacingSpecHtml: string, horizontalAuditHtml: string, verticalAuditHtml: string, specShellCss: string): void {
  ...
  assert(verticalAuditHtml.includes('aria-label="Table control row fit comparison"') && verticalAuditHtml.includes('aria-label="Plain text row"') && verticalAuditHtml.includes('class="is-control-row"') && verticalAuditHtml.includes('aria-label="Control row"'), "Expected the vertical audit to compare one plain table row with one explicit full-control row.");
  for (const control of ['aria-label="Table text input"', 'aria-label="Table number input"', 'class="bf-button"', 'class="bf-checkbox"', 'class="bf-radio"']) {
