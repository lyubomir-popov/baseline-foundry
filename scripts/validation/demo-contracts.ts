...
export function validateDemoContracts(engineSmokeHtml: string, componentShellCss: string, specShellCss: string, pageChromeCss: string, pageChromeJs: string, componentDemoJs: string, specRuntimeJs: string, examplePageJs: string): void {
  ...
  assert(pageChromeJs.includes("renderNavigationBrand()") && pageChromeJs.includes('class="bf-panel-header is-sticky is-navigation-brand"') && pageChromeJs.includes('src="${pageUrl("/demo/assets/canonical-mark.svg")}"'), "Expected shared demo chrome to compose the optional sticky tagged navigation brand from public BF primitives with a deployment-base-aware mark URL.");
  assert(pageChromeJs.includes('class="bf-panel bf-side-navigation"') && !pageChromeJs.includes('class="bf-panel bf-side-navigation bf-side-navigation-drawer"') && pageChromeJs.includes('class="bf-side-navigation-groups"'), "Expected persistent side navigation to compose the panel-owned brand inset with grouped navigation without inheriting the off-canvas drawer transform.");
...
export function validateSpacingSpecPage(spacingSpecHtml: string, horizontalAuditHtml: string, verticalAuditHtml: string, specShellCss: string): void {
  ...
  assert(iconNavigationBucket.includes('class="bf-panel-header is-navigation-brand"') && iconNavigationBucket.includes('class="bf-top-navigation-logo is-canonical-tagged"') && iconNavigationBucket.includes('src="../assets/canonical-mark.svg"') && iconNavigationBucket.includes(">Baseline Foundry<"), "Expected the horizontal audit to port Diagram Registry's tagged primary-navigation brand with the Baseline Foundry wordmark.");
  assert(iconNavigationBucket.includes("Tree child") && iconNavigationBucket.includes("Side navigation, plain") && iconNavigationBucket.includes("Side navigation, disclosure"), "Expected the brand tag, tree child, plain navigation, and disclosure navigation to remain directly comparable on the shared continuation inset.");
  ...
  assert(verticalAuditHtml.indexOf('aria-label="Chip"') < verticalAuditHtml.indexOf('aria-label="Text input"') && verticalAuditHtml.indexOf('aria-label="Tab action"') < verticalAuditHtml.indexOf('aria-label="Text input"') && verticalAuditHtml.includes('class="bf-tabs-link is-active"') && verticalAuditHtml.includes('aria-selected="true"') && verticalAuditHtml.includes('tabindex="0"') && verticalAuditHtml.includes("Active"), "Expected the standalone pill chip and explicitly initialized thick-bar active tab to appear at the front of the shared single-line comparison.");
  for (const specimen of ["Plain table cell", "Table chip", "Table status label", "Table badge", "Table link action", "Side navigation chip", "Side navigation status label", "Tab badge", "Table chip badge"]) {
  ...
  assert((verticalAuditHtml.match(/class="bf-chip[^\"]*is-nested/g) ?? []).length === 3 && (verticalAuditHtml.match(/class="bf-status-label[^\"]*is-nested/g) ?? []).length === 2 && (verticalAuditHtml.match(/class="bf-badge[^\"]*is-nested/g) ?? []).length === 3, "Expected the nested audit to cover every supported chip, status-label, and badge host combination, including a badge inside a compact table chip.");
  ...
  assert(verticalAuditHtml.includes('aria-label="Table control row fit comparison"') && verticalAuditHtml.includes('aria-label="Plain text row"') && verticalAuditHtml.includes('aria-label="Control row"') && !verticalAuditHtml.includes('class="is-control-row"'), "Expected the vertical audit to compare one plain table row with one ordinary row of explicitly nested controls.");
  for (const control of ['class="bf-input is-nested"', 'class="bf-button is-nested"', 'class="bf-checkbox is-nested"', 'class="bf-radio is-nested"']) {
    assert(verticalAuditHtml.includes(control), `Expected the table control row to include ${control}.`);
  }
  assert((verticalAuditHtml.match(/class="bf-input is-nested"/g) ?? []).length === 2, "Expected both the text and number table fields to opt into nested density.");
