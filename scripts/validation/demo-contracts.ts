...
export function validateSpacingSpecPage(spacingSpecHtml: string, horizontalAuditHtml: string, verticalAuditHtml: string, specShellCss: string): void {
  ...
  assert((verticalAuditHtml.match(/class="spacing-block-scroll"/g) ?? []).length === 3 && (verticalAuditHtml.match(/class="spacing-block-track"/g) ?? []).length === 3, "Expected the vertical audit to use three horizontally scrollable occupied-block comparison rows.");
