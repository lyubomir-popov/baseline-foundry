...
| `ROADMAP.md` | **Long-term.** Stages, parity inventory, future direction. | Agent updates rarely |
| `TODO.md` | **Active plan.** Principles, architecture, short-term tasks. | Agent updates every session |
...
2. `ROADMAP.md`
3. Repo workflow rules (`.github/copilot-instructions.md`)
...
| "Why did we make this architectural decision?" | `TODO.md` → Principles or Architecture |
...
4. Read `docs/specs.md` before changing spec-governed behavior.
...
- Sibling repos (`canonical-spacing-spec`, `canonical-specs`, `docs-typescale`, `portable-vertical-rhythm`, `brand-layout-ops`) are read-only references.
- `canonical-spacing-spec` is the primary live spec workspace for this repo.
...
- **Control padding follows the Vanilla model** — symmetric `padding-block: nudge − border`, no explicit `block-size`, margin-bottom snaps the block to the grid. No `--bf-control-block-size` var, no `.is-dense` per-control modifier. Density comes from the tier. See `TODO.md` → "Control baseline-grid invariant" for the full explanation.
...
5. Delete any other TODO/status/handoff files. One inbox + one status + one plan + one roadmap + one archive is the maximum.

## Agent file roles

- `.github/copilot-instructions.md` is the single repo-wide instruction file.
- `.github/agents/agent.md` is optional and should contain only repo-specific resume guidance.
