...
- [ ] Land the tier-and-engine refactor in stages: document the runtime class contract, keep metrics as the trusted default, and add the cap-unit engine behind the same baseline gate
...
- [ ] Broaden the cap-unit evaluation beyond the now-green `demo/components/engine-smoke.html` slice and record whether cap should replace, complement, or stay secondary to the metrics-derived engine for baseline-aligned app-tier text
