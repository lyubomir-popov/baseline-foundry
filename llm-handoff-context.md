...
- [ ] Re-test whether the downstream `viewer-panel__content` stage-centering shell should stay local composition or deserves a tiny upstream helper once the swap is further along, especially now that a real `brand-layout-ops` bug showed a long pinned inspector can stretch the stage row unless the consumer hard-clamps the shell to the viewport
