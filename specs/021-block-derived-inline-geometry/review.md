...
**Status**: Implemented, independently re-reviewed, and green on
`feat/021-block-derived-inline-geometry`; owner/stakeholder acceptance pending
...
| nested icon-only button, before | 48×32 | 48×24 | 48×24 | 48×20 |
| nested icon-only button, after | 32×32 | 24×24 | 24×24 | 20×20 |
...
| bordered nested icon | direct | direct | spacing | spacing |
...
target shape. This matters for App's 24×24 nested icon and notification close:
their 2px rounded corners make them undersized, so they use spacing rather than
being misreported as direct passes. “Spacing” is proven pairwise: the browser
...
- Browser behavior sweeps all four tiers and both tones for circles, stadiums,
  ...
  table-hosted interactive nested-chip target spacing, standalone/nested/link/
  specialized icon squares, real page-chrome buttons, pagination ownership,
