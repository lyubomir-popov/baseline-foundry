...
- [ ] T004 [US1] Set derived caps in `config/tiers/*.json` and document the coupled tier/density model in `README.md` and `docs/architecture.md`.
- [ ] T005 [US1] Add explicit monotonic/direct/scoped token assertions in `scripts/validate-build.ts`.
- [ ] T006 [US1] Extend `scripts/verify-component-behavior.ts` to compare wide direct/scoped fixed rows and prove the App page remains fluid.

## Phase 3: User Story 2 — Metric-only document navigation

- [ ] T007 [US2] Move TOC link rhythm to list/item gaps in `src/css-components/document-navigation.ts`.
- [ ] T008 [US2] Correct the confirmed in-page-navigation desktop/expanded link defect while preserving compact control padding.
- [ ] T009 [US2] Add static and browser padding/gap/focus/wrap/RTL assertions in `scripts/validation/renewal-component-contracts.ts` and `scripts/behavior/ported-component-contracts.ts`.

## Phase 4: User Story 3 — Following-item dividers

- [ ] T010 [US3] Reposition the divided-list pseudo-rule in `src/css-components/static-content-ports.ts` without changing item boxes or the 24px gap.
- [ ] T011 [US3] Replace the centred-divider static/browser assertions with the final-half-rem geometry contract.

## Phase 5: Validation and review

- [ ] T012 Regenerate source outputs with `npm run build` and run focused static, behavior, and component tests.
