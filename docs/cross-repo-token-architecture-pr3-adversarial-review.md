...
| F3 | Medium | BF and Canonical can both emit the unnamespaced Canonical property set; import order and nested product scopes were neither tested nor documented. | Define aligned product classes as the co-loading contract and test the provider CSS shape before/after BF plus nested Docs/App/OS scopes. |
