...
- The next parity slice has started landing as preview surfaces rather than abstract TODOs: chips, badges, status labels, tables, search box, and search-and-filter now each have their own saved demo page and generated CSS selectors, but they have not yet been promoted into the automated baseline gate because their rhythm math still needs cleanup.
...
- The next high-value borrow area is explicit spacing invariants promoted into stronger repo-level validation.
