...
- The latest Canonical refresh now also directly governs spacing in the grid layer: container queries decide only the column count, while viewport breakpoints decide gutters and outer margins (`16px` x-small, `24px` gutters from `620px`, `32px` outer margins from `1036px`).
...
- The next downstream panel tranche is now upstream too: equal-width dense panel tabs, selectable preset/output rows, and style/mapping palette cards all have package-side selectors plus downstream aliases (`config-tabs`, `output-profile-tabs`, `preset-radio-row`, `style-palette`) so they can be removed from `brand-layout-ops` local CSS instead of living only in the audit report.
...
- The downstream style audit now has a sharper boundary: drawer and pinned-aside resize behavior are replaceable now from `baseline-foundry`, and the former "missing" dense panel slice is upstream too: equal-width dense tabs, preset radio rows, style-palette cards, tight help-text modifiers, compact color input sizing, the operator-selector micro-pattern, fill-height panel behavior, checkbox-field density handling, and dense action-row overflow helpers are all shipped package-side.
...
- The next high-value borrow area is now the real downstream swap test itself plus any final spacing invariants that still need to be promoted into stronger repo-level validation. The biggest remaining discussion point from the audited stylesheet is whether the stage-centering `viewer-panel__content` shell deserves a canonical helper or should stay as downstream composition.
