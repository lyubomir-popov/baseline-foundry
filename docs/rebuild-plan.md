...
- The downstream style audit now has a sharper boundary: drawer and pinned-aside resize behavior are replaceable now from `baseline-foundry`, and the former "missing" dense panel slice is upstream too: equal-width dense tabs, preset radio rows, style-palette cards, tight help-text modifiers, compact color input sizing, and the operator-selector micro-pattern are all shipped package-side. The remaining downstream discussion points are smaller shell helpers like fill-height panel behavior, checkbox-field density overrides, and action-row overflow handling.
...
- Browser QA now checks behavior as well as rhythm: `npm test` includes a Playwright resize-flow test so the pinned-aside shell cannot silently lose drag, keyboard, reset, or persistence behavior while the CSS still looks plausible.
...
- The next high-value borrow area is now the small remaining downstream shell/helpers after the big panel parity slice: fill-height panel behavior, checkbox-field density overrides, action-row overflow helpers, and any spacing invariants that still need to be promoted into stronger repo-level validation.
