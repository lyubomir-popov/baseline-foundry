...
- [x] Evaluate whether the `pragma` cap-unit approach actually simplifies baseline-aligned text in container-owned app layouts; the cap engine is now the default across both the main CSS and the compat layer — routing variables (`--bf-selected-start-nudge`, `--bf-selected-end-nudge`) are emitted per-element defaulting to cap, with `.bf-engine-metrics` as the opt-in override
...
- The current rollout rule is strict: the cap engine is now the default. The routing variables (`--bf-selected-start-nudge`, `--bf-selected-end-nudge`, `--bf-semantic-space-after`) are emitted per-element in each text role rule, defaulting to cap-unit values. The previous root-level routing was invalid — CSS custom properties resolve `var()` at the element where the property is declared, so routing variables declared on the `.bf-theme` root resolved to their fallback `0rem` because the source variables only existed on descendant text elements. Moving routing per-element fixes the resolution bug and also makes cap the natural default since cap computes nudges purely from CSS runtime (`1cap` unit + `mod()`). The `.bf-engine-metrics` class is preserved as an opt-in override that sets per-element`--bf-selected-start-nudge: var(--bf-metrics-start-nudge)` on all text selectors under it. The compat layer follows the same pattern: `roleAlignmentVars()` defaults to cap, and `.bf-engine-metrics` on the theme root overrides to metrics.
