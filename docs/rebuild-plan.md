...
- The downstream style audit now has a sharper boundary: drawer and pinned-aside resize behavior are replaceable now from `baseline-foundry`, while equal-width dense tabs, preset radio rows, style-palette cards, tight help-text modifiers, compact color input sizing, and the operator-selector micro-pattern are the clearest remaining "no shipped analog yet" discussion points.
