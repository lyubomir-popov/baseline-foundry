...
- [ ] Evaluate whether the `pragma` cap-unit approach actually simplifies baseline-aligned text in container-owned app layouts; the baseline-gated `demo/components/engine-smoke.html` page is now the first green app-tier comparison surface, and the next step is to see whether harder specimens keep cap baseline-safe enough to replace, complement, or stay secondary to the metrics engine
