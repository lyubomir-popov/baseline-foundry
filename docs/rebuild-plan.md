...
- The next parity slice has started moving past pure form/data surfaces: list-tree, contextual-menu, and tooltip now have their own runtime modules, generated CSS, saved component pages, and baseline-gated demos instead of existing only in `portable-vertical-rhythm`.
...
- The next high-value borrow area is the remaining overlay-adjacent and utility families, especially code-snippet and any downstream-specific overlays, plus any spacing invariants that still need to be promoted into stronger repo-level validation.
